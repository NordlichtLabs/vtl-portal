...
if choice == "VTL Generator":
    ...
    with col2:
        ...
        # Funktion für Überschrift + Datum in einer Zeile
...
for idx, item in enumerate(st.session_state.history_data):
    ...
    if st.session_state.selected_hist_idx == idx:
        st.markdown(f"<div class='detail-box'><b>DE:</b> {item['DE']}<br><b>AT:</b> {item['AT']}<br><b>IT:</b> {item['IT']}<br><br><span style='font-size:10px;'>Hash: {item['Hash']}</span></div>", unsafe_allow_html=True)    st.title("🔍 Public Validator")
    cert_id = st.text_input("Zertifikats-ID oder Hash eingeben", key="val_input")

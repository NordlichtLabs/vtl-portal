...
st.markdown("""
    ...
    /* How it works Cards (Blau ohne Schatten/Linie) */
    ...
    if st.button("Zahlen berechnen & Zertifikat erstellen"):
        if st.session_state.registered_salts:
            ...
            with res_r:
                ...
                st.markdown(f"""<div class='certificate'><div class='verified-seal'>VTL VERIFIED</div><h3 style='margin-top:0; font-size:16px;'>VTL AUDIT CERTIFICATE</h3><p style='font-size:12px;'><b>ENTITY:</b> {c_name}<br><b>REF-ID:</b> {p_id}<br><b>DATE:</b> {today_str}</p><hr style='border:1px solid #eee;'><p style='font-size:10px; word-break:break-all;'><b>MASTER HASH (PROTOCOL PROOF):</b><br><b>{m_hash}</b></p><p style='font-size:10px; word-break:break-all;'><b>VAULT REFERENCE (SALT HASH):</b><br><b>{curr_sh}</b></p><hr style='border:1px dashed #000;'><p style='text-align:center; font-size:22px; font-weight:bold; letter-spacing:2px;'>{res_str}</p></div>""", unsafe_allow_html=True)
